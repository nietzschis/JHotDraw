...
public class DefaultDrawingView
    ...
    public void flip(String actionCommand) {
        ...
        if (Objects.equals(actionCommand, "Horizontal")){
            tx = AffineTransform.getScaleInstance(-1,1); //inverts, but goes out of bounds of canvas        
            tx.translate(-(getWidth()), 0); //minus width of canvas so figure returns to canvas.      
        } else if (Objects.equals(actionCommand, "Vertical")){
            tx = AffineTransform.getScaleInstance(1, -1); //inverts, but goes out of bounds of canvas        
